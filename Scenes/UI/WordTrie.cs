...
public class WordTrie : Node{
    ...
    public void insert(string word, string _entryId = "", List<string> _synsetIds = null, string _rootForm = "") {
        TrieNode node = rootNode;

        for(int i = 0; i < word.Length; i++) {
            char currentLetter = word.ToUpperInvariant()[i];
        ...
        if(!node.entSyns.ContainsKey(_entryId)){ 
            node.entSyns.Add(_entryId, _synsetIds);
        }
        node.rootForm = _rootForm;
    }
    ...
    public List<string> search(string subword) {
        ...
        for(int i = 0; i < subword.Length; i++) {
            char currentLetter = subword.ToUpperInvariant()[i];
            if(node.children.ContainsKey(currentLetter)){
                node = node.children[currentLetter];
...
public class TrieNode {
    ...
    public Dictionary<string, List<string>> entSyns = new Dictionary<string, List<string>>();
    public int depth = 0;
    public string word = "";
...
public class RootComparer : IEqualityComparer<char> {
    ...
    //Removed Serialization Code
    // using System.Runtime.Serialization;
    //     public TrieNode(SerializationInfo info, StreamingContext ctxt) {
    //     val = (char)info.GetValue("val", typeof(char));
    //     depth = (int)info.GetValue("depth", typeof(int));
    //     word = (string)info.GetValue("word", typeof(string));
    //     isRootNode = (bool)info.GetValue("isRootNode", typeof(bool));
    //     completeString = (bool)info.GetValue("completeString", typeof(bool));    
    //     children = (Dictionary<char, TrieNode>)info.GetValue("children", typeof(Dictionary<char, TrieNode>));
    //     entryId = (string)info.GetValue("entryId", typeof(string));
    //     synsetIds = (List<string>)info.GetValue("synsetIds", typeof(List<string>));
    // }
    ...
    //     info.AddValue("isRootNode", isRootNode);
    //     info.AddValue("completeString", completeString);    
    //     info.AddValue("children", children, typeof(Dictionary<char, TrieNode>));
    //     info.AddValue("entryId", entryId);
    //     info.AddValue("synsetIds", synsetIds, typeof(List<string>));
    // }
    //  public void GetObjectData(SerializationInfo info, StreamingContext ctxt) {
