...
public class UI : Control {
    ...
    public override void _Ready(){
        ...
        //wordNet = GetNode<WordNet>("../NLP/WordNet");
    ...
    public override void _Input(InputEvent @event){
        if (@event is InputEventKey key && playerLine.Text.Length > 0){//If Text is empty it'd crash
            if(wordList.Visible && helperState == HELPER.LONETRIE) {
                ...
                if(key.IsActionReleased("ui_left")){//If it's held down for ever the list
                    if(playerLine.CaretPosition <= detales[0][5]-1) {//Stays in place
